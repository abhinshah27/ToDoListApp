...
class MainVMDeleteTask : BaseTest() {
    ...
    fun listTaskSuccess() = runTest {
        Mockito.`when`(dataSource.deleteTask(any(), any())).thenAnswer {
            val callback: DatabaseOperationInterface = it.arguments[1] as DatabaseOperationInterface
            callback.deleteTask(SUCCESS)
        }

        mainViewModel.resultDeleteTask.test {
            mainViewModel.deleteTask(TaskEntity(1, "", "", "", 11))
