...
import com.todo.example.utils.SUCCESS
...
import com.todo.example.utils.DUMMY_DATE
...
class MainVMAddTask : BaseTest() {
    ...
    fun addTaskSuccess() = runTest {
        Mockito.`when`(dataSource.insertTask(anyOrNull(), any())).thenAnswer {
            val callback: DatabaseOperationInterface = it.arguments[1] as DatabaseOperationInterface
            callback.addTask(SUCCESS)
        }

        mainViewModel.resultAddTask.test {
            mainViewModel.addTaskList("", DUMMY_DATE, "")
